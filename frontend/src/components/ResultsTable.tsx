...
export const ResultsTable = ({ data }: { data: ProcessedPart[] }) => {
  ...
  const filtered = useMemo(() => data.filter((r) => (`${r.partNumber} ${r.manufacturer} ${r.aecStandard} ${r.aecStatus}`).toLowerCase().includes(query.toLowerCase())), [data, query]);
  const columns = useMemo(() => [
    ...
    h.accessor('confidence', { header: 'Confidence', cell: i => `${Math.round(i.getValue() * 100)}%` }),
    h.accessor('source', { header: 'Source', cell: info => info.getValue() ? <a className="text-wine underline" href={info.getValue()} target="_blank">Reference</a> : '—' }),
    h.accessor('notes', { header: 'Notes' })
  ], []);

  const table = useReactTable({ data: filtered, columns, getCoreRowModel: getCoreRowModel() });

  return <div className="glass rounded-luxury p-4 overflow-auto space-y-3">
    <input className="w-full md:w-80 rounded-full px-4 py-2 border border-mauve bg-white/70" placeholder="Search part, mfr, or AEC standard..." value={query} onChange={(e) => setQuery(e.target.value)} />
